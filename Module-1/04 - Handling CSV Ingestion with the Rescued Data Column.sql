...
-- MAGIC %python
-- MAGIC working_dir = "/Volumes/dbacademy/ops/labuser101"  # Replace with the actual path in DBFS
-- MAGIC
-- MAGIC spark.sql(f'''
-- MAGIC     SELECT *
-- MAGIC     FROM text.`{working_dir}/csv_demo_files/malformed_example_1_data.csv`
-- MAGIC ''').display()
...
SELECT *
FROM read_files(
        '/Volumes/dbacademy/ops/labuser101' || '/csv_demo_files/malformed_example_1_data.csv',
        format => "csv",
        sep => "|",
        header => true
      );
...
-- MAGIC spark.sql(f'''
-- MAGIC   SELECT *
-- MAGIC   FROM text.`/Volumes/dbacademy/ops/labuser101/csv_demo_files/malformed_example_2_data.csv`
-- MAGIC ''').display()
...
SELECT *
FROM read_files(
        '/Volumes/dbacademy/ops/labuser101' || '/csv_demo_files/malformed_example_2_data.csv',
        format => "csv",
        sep => "|",
        header => true
      );
