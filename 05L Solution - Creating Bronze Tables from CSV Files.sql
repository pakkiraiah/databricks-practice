...
VALUES ('/Volumes/dbacademy/ops/labuser101')
...
values('/Volumes/dbacademy/ops/labuser101 '|| '/csv_demo_files/lab_malformed_data.csv')
...
-- MAGIC
-- MAGIC     Copy the path from the output below and paste it within the backticks in the query to reference the **lab_malformed_data.csv.csv** file.
-- MAGIC
...
-- MAGIC spark.sql(f'''
-- MAGIC SELECT *
-- MAGIC FROM text.`/Volumes/dbacademy/ops/labuser101/csv_demo_files/lab_malformed_data.csv`
-- MAGIC '''
-- MAGIC ).display()
...
SELECT * 
FROM read_files(
        '/Volumes/dbacademy/ops/labuser101' || '/csv_demo_files/lab_malformed_data.csv',
        format => "csv",
        sep => ",",
        header => true,
        schema => '''
...
-- MAGIC %python
-- MAGIC # Builds the final lab table for students to use as a resource
-- MAGIC query_1 = """
-- MAGIC DROP TABLE IF EXISTS 05_lab_solution
-- MAGIC """
-- MAGIC
-- MAGIC query_2 = f"""
-- MAGIC CREATE TABLE 05_lab_solution 
...
-- MAGIC         CONCAT('/Volumes/dbacademy/ops/labuser101', '/csv_demo_files/lab_malformed_data.csv'),
-- MAGIC         FORMAT => "csv",
-- MAGIC         SEP => ",",
-- MAGIC         HEADER => true,
...
-- MAGIC """
-- MAGIC
-- MAGIC # Execute the queries
-- MAGIC spark.sql(query_1)
-- MAGIC spark.sql(query_2)
